...
for i = 1: 4
    ...
    while flag ~=1
        ...
        newVoltage = max(voltage(V)) 
        if newVoltage > 0 % AP fired
            if newVoltage - 0.0001 < 0 % find the minimum value
                amplitudes = [amplitudes, Iamp];
                flag = 1; 
                break; % break the while loop hopefully? 
                
            else 
                
                % TODO: add percent error here
                % resetting counters because amp needs to be smaller
